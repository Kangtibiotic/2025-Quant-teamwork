...
ins_path = os.path.join("../../raw_data", "FS_Comins.csv")
...
# 读取财务数据
ins = pd.read_csv(ins_path, usecols=['Stkcd', 'Accper', 'B001100000']).rename(columns={'B001100000': 'Revenue'})
ins['Accper'] = pd.to_datetime(ins['Accper'])
ins = ins.sort_values(['Stkcd', 'Accper'])

ins['Revenue_q'] = ins.groupby(['Stkcd', ins['Accper'].dt.year])['Revenue'].diff()
is_q1 = ins['Accper'].dt.month == 3
ins.loc[is_q1, 'Revenue_q'] = ins.loc[is_q1, 'Revenue']
ins['Revenue_TTM'] = ins.groupby('Stkcd')['Revenue_q'].rolling(4, min_periods=4).sum().reset_index(0, drop=True)

ins['Revenue_TTM_lag4'] = ins.groupby('Stkcd')['Revenue_TTM'].shift(4)
ins[factor_name] = (ins['Revenue_TTM'] - ins['Revenue_TTM_lag4']) / ins['Revenue_TTM_lag4'].abs()

# 财务数据滞后
ins['PublicationDate'] = ins['Accper'].apply(get_pub_date)
df_fin = ins[['Stkcd', 'PublicationDate', factor_name]].dropna().sort_values(['PublicationDate', 'Stkcd'])
