...
def plot_ic_analysis(ic_series, ic_stats, factor_name, figsize=(15, 12)):
    ...
    plt.tight_layout()
    plt.savefig(fr'results/单因子测试results/{factor_name}_ic.png')
    plt.show()
...
def plot_grouping_results(results, figsize=(14, 10)):
    ...
    plt.savefig(fr'results/单因子测试results/{factor_name}_grouping.png')
    plt.show()


if __name__ == "__main__":
    # 读取日频交易数据
    daily_ohlc_files = os.listdir("processed_data/TRD_Dalyr")
    daily_ohlc_list = []
    for file in tqdm(daily_ohlc_files, desc="Loading data"):
        daily_ohlc = pd.read_csv(
            os.path.join("processed_data/TRD_Dalyr", file), dtype={"Stkcd": str}
        )
    ...
    summary_df.to_csv(os.path.join(output_dir, "单因子测试results.csv"), index=False, encoding="utf-8-sig")
    print(f"\n✅ 所有因子测试完毕，结果已保存到：{os.path.join(output_dir, '单因子测试results.csv')}")
