...
df_merged = pd.merge(factors, monthly_ohlc[['Stkcd', 'month', 'ret']], left_on=['Stkcd', 'use_month'],
                     right_on=['Stkcd', 'month'])
...
if results_list:
    ...
    # 保存结果
    os.makedirs('results/train_results', exist_ok=True)
    final_results['long_short'] = final_results[f'Group_{n_groups}'] - final_results['Group_1']
    output_filename = 'results/train_results/lightGBM_returns.csv'
    ...
    plt.title('Decile Portfolios Cumulative Returns (Pre-2023 lightGBM Regression)',
    ...
    output_img = 'results/plots/lightGBM_train_cumulative_returns_plot.png'
    ...
    # 保存性能统计
    performance_df.to_csv('results/train_results/lightGBM_performance_stats.csv')
    print("\n性能统计已保存至: results/train_results/lightGBM_performance_stats.csv")
